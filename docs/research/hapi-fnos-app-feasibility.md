...
| HAPI Web 在 fnOS 服务器 B、Hub 在 Ubuntu 服务器 A | **Go** | Web 容器作为反向代理；Ubuntu 提供仅 fnOS 可达的 LAN endpoint，首选 TLS 代理到 loopback Hub。 |
...
- 生产 fnOS 版本必须强制使用应用公开地址的同源代理，不在浏览器端设置 Ubuntu Hub LAN URL。
- Ubuntu Hub 地址属于容器服务端配置，由反向代理读取。
- 生产构建必须忽略或拒绝 `?hub=`，首次运行时清除并永久忽略旧的 `localStorage.hapi_hub_url`，同时移除 Hub picker。不能只把选择器藏起来而继续读取覆盖值。

这是凭据边界，不只是简化 UI。当前 `?hub=` 会在鉴权前持久化；攻击者若诱导用户打开带恶意 Hub 的链接，用户随后输入的长期 `CLI_API_TOKEN` 会被 Web 发往该地址。fnOS 包应让浏览器目标不可配置，只有管理员能在容器服务端配置固定 Ubuntu 上游。

`VITE_HUB_PROXY` 只服务 Vite 开发服务器，不能解决 fnOS 生产部署。生产环境应由容器启动配置生成反向代理上游，前端继续使用页面自身 origin；不需要把 Ubuntu Hub 地址编译进 Web。[H4]

官方 Pages 工作流使用 `VITE_REQUIRE_HUB_URL=1` 构建，目标正是让用户选择远程 Hub。fnOS 包不能直接复用该产物；应从与 Hub 对应的 source commit 单独构建固定同源版本。[H1]
...
Hub 默认只监听 `127.0.0.1:3006`。Docker 容器中的 `127.0.0.1` 是容器自身，也不是 Ubuntu 主机。最稳妥的做法是在 Ubuntu 上增加只面向 LAN 的 TLS 反向代理，由它转发到仍监听 `127.0.0.1` 的 Hub；较简单的备选才是让 Hub 监听 LAN 地址或 `0.0.0.0`，并用主机防火墙只允许 fnOS 地址访问。若 Hub 只绑定 Ubuntu 的 LAN IP，同机 CLI/Runner 默认访问 `localhost:3006` 会失效，必须同步设置其 `HAPI_API_URL`。[H6][H10]
...
| PWA/通知 | 部分 scope 已跟随 base，但通知图标、Web Push 和 ServerChan 的 `/sessions/:id` 深链仍有根路径假设 | 所有图标、通知点击、分享入口和 Hub 生成的深链保持在 scope 内。 |
...
Hub 的 `/health` 返回 `protocolVersion`；CLI 的部分请求失败路径可以提示协议版本差异，但这不是主动握手。Web 目前只在 About 页面展示自己编译时的版本，没有主动比较 Hub `/health`。[H8]
...
| fnOS HAPI 容器 | 静态 Web、SPA fallback、反向代理、缓存和连接超时控制；转发登录时会看到请求体中的原始 `CLI_API_TOKEN` | 不运行 Hub，不保存 Hub SQLite，不应持久化或记录 `CLI_API_TOKEN`。 |
...
- 根路径入口也要处理语音公开 URL。Gemini/Qwen token API 返回的 WebSocket 地址来自进程环境变量 `HAPI_PUBLIC_URL` 或 Hub 所见 request origin；若后者是 Ubuntu LAN 地址，iPhone 会回连失败或触发 Mixed Content。[H7]
...
6. Web Push、ServerChan 等通知深链改为 `/app/hapi/sessions/:id`；Telegram `startapp` 也需在真实 iOS 入口验证。[H7]
...
2. 普通 `/api/*` 的读取超时不少于 5 分钟并留出余量；部分模型查询 RPC 可运行 120 秒，下载准备 RPC 可运行 300 秒。还要实测 fnOS 网关和 FN Connect 不会更早截断。[H10]
3. `/api/events` 关闭响应缓冲，读取超时覆盖长连接；Hub 每 30 秒心跳，Web 90 秒无活动才判定失联。
4. `/socket.io/` 同时支持 HTTP polling 和 WebSocket Upgrade。
5. 同一个 Engine.IO path 承载浏览器 `/terminal` 和 CLI `/cli` 两个 namespace。外部暴露 `/socket.io/` 会同时暴露两者，普通路径反向代理无法在升级后按 namespace 隔离；必须保留两边各自鉴权，增加握手速率/连接数限制，长期可评估拆分 Engine.IO path。[H5]
6. `/api/voice/gemini-ws`、`/api/voice/qwen-ws` 支持 WebSocket Upgrade。
7. 请求体上限至少覆盖 Hub 的 68 MiB；HAPI 单文件业务限制为 50 MiB。
8. 两种入口形态都禁止代理和 Service Worker 缓存认证 API 响应；哈希静态资源可长期缓存，`index.html`、`sw.js` 和 manifest 不长期缓存。
9. SSE 可定向脱敏 `token`；语音 WebSocket 必须隐藏整个 query string，因为其中还有 `systemPrompt`、voice、language 等内容，并要为编码后的 prompt 设置和验证请求行大小上限。[H7]
10. Web 服务健康与 Ubuntu Hub 可达分开。Hub 临时离线只显示连接错误，不触发 fnOS 无限重启容器。
11. 保留 HAPI 自身 JWT/namespace 鉴权。FN Connect/fnOS 登录态只保护入口，不能直接映射为 HAPI 权限。
12. 在 fnOS 入口完成管理员 Header 校验后，转发到 Ubuntu Hub 前移除不需要的 fnOS 会话 Cookie 和 `X-Trim-*` 用户 Header；保留 HAPI `Authorization`、查询 JWT 和 WebSocket Upgrade 所需 Header，避免把 NAS 登录凭据扩散到另一台主机。

语音链路并不完全相同：Gemini/Qwen 是 `iPhone -> fnOS Proxy -> Ubuntu Hub WebSocket -> 第三方服务`；ElevenLabs 则由 Hub 先取 conversation token，再由 iPhone 直接建立到 ElevenLabs 的 WebRTC 会话。验收既要测试 Ubuntu 的第三方 API/WebSocket 出网，也要测试 iOS 飞牛 WebView 的麦克风权限、WebRTC 和蜂窝网络出网。[H7]

### 11.6 Ubuntu Hub 的 LAN 配置

- 首选：Ubuntu 上的 LAN TLS 反向代理监听固定内网地址，Hub 保持默认 `127.0.0.1`；防火墙只允许 fnOS 服务器 B 访问该代理。
- 备选：Hub 直接以 `HAPI_LISTEN_HOST=0.0.0.0` 或固定 LAN IP 监听，并由防火墙只放行 fnOS 固定 IP。若只绑定 LAN IP，同机 CLI/Runner 也要把 `HAPI_API_URL` 改为该地址；否则默认 `localhost` 连接会断开。[H10]
- `CORS_ORIGINS` 加入实际 FN Connect origin 和本地 fnOS App origin，以通过 Hub 的 Socket.IO Origin 检查。
- 根路径入口下，Ubuntu Hub 进程的环境变量 `HAPI_PUBLIC_URL` 应指向用户实际打开的外部 FN Connect HTTPS 地址，而不是 Ubuntu LAN URL。语音路由直接读 `process.env.HAPI_PUBLIC_URL`，只改已落盘 `settings.json` 的 `publicUrl` 不生效。[H7]
- 本地 fnOS 地址和 FN Connect 地址若是两个 origin，一个静态 `HAPI_PUBLIC_URL` 无法同时让两边就近回连。首版可指定 FN Connect 为唯一 canonical origin；更完整的方案是修改语音 URL 生成逻辑，基于可信代理传来的外部 origin 或返回相对同源 URL。统一网关形态还必须保留 `/app/hapi` pathname。
...
本方案不使用 HAPI Relay、Cloudflare Tunnel 或额外公网 Hub 端口。若未来改变这一目标，应作为另一套公网暴露方案单独做威胁建模，不能与当前 fnOS/FN Connect 方案混用。
...
- `allUsers=false` 只控制 fnOS 桌面入口的可见性，不等于访问控制。统一网关形态应只信任网关注入的 `X-Trim-Isadmin` 并拒绝非管理员；端口入口则必须测试用户能否用直接 URL 绕过桌面隐藏。无论哪种形态都保留 HAPI JWT。
- 不因统一网关已校验 NAS 登录就跳过 HAPI JWT/namespace 鉴权。
- 不在安装向导、Compose、静态配置或应用日志保存 `CLI_API_TOKEN`。
- Hub 端口不对整个 LAN 或公网无差别开放。

### 12.2 浏览器凭据、缓存与共享 Origin 风险
...
当前 HAPI 会持久化长期 `CLI_API_TOKEN`，Service Worker 还会按 URL 缓存部分已认证 API 响应，缓存键没有 Hub/namespace 维度。[H4][H6]

无论根路径还是统一网关入口，fnOS 发行版都应：

- 不持久化原始 `CLI_API_TOKEN`；优先改为内存短会话、重新登录，或设计服务端配对凭据。
- 禁用 sessions/machines 等认证 API 的离线缓存，只缓存静态 shell，避免切换 namespace 或用户后离线串读。
...
fnOS 代理也会在转发 `POST /api/auth` 时看到原始 token。因此 fnOS 主机、Web/Proxy 镜像、容器运行配置以及 fnOS 到 Ubuntu 的 LAN/TLS 链路都属于可信计算基；正确承诺是“代理不持久化、不记录 token”，而不是“代理接触不到 token”。
...
- SSE JWT 位于查询参数，语音 WebSocket JWT 也位于查询参数。SSE 日志至少对 `token` 脱敏；语音 URL 还带 `systemPrompt`、voice、language 等信息，fnOS 网关、容器代理和 Hub 日志应隐藏整段 query string，并测试过长请求行的拒绝行为。
...
| 高 | fnOS 构建仍接受 `?hub=` 或已存 Hub 覆盖 | 恶意链接可把用户输入的长期 token 发到攻击者 Hub | 生产版强制同源，禁用参数、存储项和 Hub picker，不复用 Pages 产物。 |
| 高 | 长期 token 持久化、认证 API 缓存及统一网关共享 origin | Hub 高权限凭据或会话数据泄漏 | 不持久化 seed token；两种入口都禁用认证 API 缓存；优先独立 origin。 |
| 高 | 为 fnOS 开放 Ubuntu LAN 服务扩大攻击面 | 局域网其他主机可尝试访问 Hub | 首选 LAN TLS 代理到 loopback Hub；否则防火墙仅放行 fnOS，保留强 token。 |
...
| 高 | 对外 `/socket.io/` 同时承载 `/terminal` 和 `/cli` | CLI 长期 token 的鉴权入口也进入 FN Connect 暴露面 | 保留 namespace 鉴权、限制握手速率/连接数；长期评估拆分 Engine.IO path。 |
| 高 | `allUsers=false` 被误当作权限控制 | 普通 NAS 用户或直接 URL 绕过入口隐藏 | 统一网关校验可信管理员 Header；端口入口做绕过测试；始终保留 HAPI JWT。 |
...
5. 锁屏、后台 30 秒/5 分钟、WKWebView 被系统回收后重开、Wi-Fi/蜂窝切换时，SSE/WS 与 FN Connect 登录会话能否恢复。

若根路径入口全部通过，采用根路径代理形态；否则采用官方统一网关前缀形态。若两种形态在 FN Connect 中继下都无法稳定转发长连接，则该目标 No-Go，需要与飞牛确认平台支持后再继续。

### Gate 1：HAPI Web 与代理能力

两种入口形态共同通过：

- 生产构建忽略恶意 `?hub=`，清理旧 `hapi_hub_url`，界面没有 Hub picker；登录请求只能发往同源代理。
- REST 全功能和 5 分钟长耗时请求；两条 SSE 与终端 Socket.IO 的 polling -> WebSocket upgrade。
- 锁屏、后台 30 秒/5 分钟、WKWebView 回收、Wi-Fi/蜂窝切换后，两条 SSE、终端 Socket、JWT 刷新与 FN Connect 会话分别恢复，且不会重复提交操作。
- Gemini/Qwen 经 Hub WebSocket、ElevenLabs 经 iPhone WebRTC 均可用；验证麦克风授权、Ubuntu 出网、iPhone 出网及本地/FN Connect 两个 origin 的回连地址。
- 登录或切换 namespace 后无旧认证 API 缓存串读，离线只返回静态 shell。

统一网关前缀形态还必须通过：

- `/app/hapi/` 登录、会话列表与 `/app/hapi/sessions/:id` 直接刷新。
- Gemini/Qwen 语音 WebSocket 回连 URL 包含正确前缀。
...
- 防火墙证明除 fnOS 外无法访问 Ubuntu 的 Hub/LAN 代理端口；同机 CLI/Runner 在所选监听方案下保持连接。
- 代理和网关日志确认不落完整 JWT/CLI token，也不落语音 query 中的 prompt、voice、language。
- 统一网关缺失或伪造 `X-Trim-Isadmin` 时拒绝访问；普通 NAS 用户与端口直达 URL 均不能绕过管理员入口策略。
- `/socket.io/` 的 `/cli` 与 `/terminal` 鉴权、握手速率和连接数限制经过外部入口测试。
...
2. **根路径入口可用时**：无需子路径改造，但仍须完成固定同源、认证缓存禁用、语音公开 URL 和长连接代理配置。
